...
//		StackReg
// 		MulitstackReg
//		Attentuation Correction (optional)
...
#@ boolean(label="3. Perform cell detection and analysis?", description="") CellAnalysisON
#@ boolean(label="    Genearate cell analysis images and heatmaps?", description="") CreateCellAnalysisVisON
#@ boolean(label="4. Perform mesoscale mapping projection analysis?", description="") ProjectionTransformationON
    ...
    if (File.exists(inputdir)) {
        if (File.isDirectory(inputdir)) {
			...
			if (File.exists(inputdir + "/Analysis_Settings.csv")) {
       			...
       			exit("Analysis Settings file doesn't exist, please run Set Analysis Settings step for this folder");
	   		}
...
if (AtlasAnalysisON == true ||  ProjectionTransformationON == true || CellAnalysisON ==true || CreateCellAnalysisVisON ==true || CreateColorDensityImagesON == true || IntensityMeasureInABAON == true) {
...
// Can be run internally within ImageJ but creating bat file can be used for troubleshooting elastix and install errors
	AAstarttime = getTime();
...
if ((CellAnalysisON == true && CellDetMethod == "Machine Learning Segmentation with Ilastik") || (ProjectionTransformationON == true && ProjDetMethod == "Machine Learning Segmentation with Ilastik")) {
		...
		//Prepare Ilastik commands/ // stack by stack times out so have to process slice by slice could potentially process substacks
...
function AnnotatePoints (PointsIn, AtlasAnnotationImg, AtlasAnnotationCSV, MeasuredIntCSV, AnnotatedSummary, AnnotatedOut) {
	...
	print(WriteOut, "X,Y,Z,Z_Dither,Mean_Int_Ch1,Mean_Int_Ch2,Mean_Int_Ch3,Mean_Int_Ch4,Hemisphere,Region_ID\n");
	...
	// 3) Print headings
	print(WriteOut,"Total_Cells_Right,Total_Cells_Left,Region_ID,Region_Acronym,Region_Name_1,Region_Name_2,Region_Name_3,Parent_ID,Parent_Acronym"); 

	if (Modified_IDs == "true") {
		RegionIDs = OutputRegionIDs;
	}
	for(i=0; i<RegionNames.length; i++){ 
		RegionNamesArray = newArray(" ", " ", " ");
		RegionNames123=split(RegionNames[i],",");
		for (j=0; j<RegionNames123.length; j++){ 
			if (j <= 2 ){
				RegionNamesArray[j] = RegionNames123[j];
			} else if (j > 2) {
				RegionNamesArray[2] = RegionNamesArray[2] + RegionNames123[j];
			}
		}
		// remove comma from region acronyn name
		RegionAcronym = getResultString("acronym", i);
		RegionAcronym = replace(RegionAcronym, ",", " ");

		print(WriteOut, RegionCountsRight[i]+","+RegionCountsLeft[i]+","+RegionIDs[i]+","+RegionAcronym+","+RegionNamesArray[0]+","+RegionNamesArray[1]+","+RegionNamesArray[2]+","+getResultString("parent_ID", i)+","+getResultString("parent_acronym", i));
	} 
...
function AnnotatePointsSpinalCord (PointsIn, AtlasAnnotationImg, AtlasAnnotationCSV, MeasuredIntCSV, AnnotatedSummary, AnnotatedOut, AnnotatedSegmentSummary) {
	// Fix for column issues in Fiji
	run("Input/Output...", "jpeg=100 gif=-1 file=.csv use use_file save_column");
	OpenAsHiddenResults(PointsIn);
	ChCount = nResults;
	setBatchMode(true);
	...
	for (i = 0; i < ChCount; i++) {
		...
		ZPos[i] = Z;
		// Dither Z to reduce clustering in cases where sectioning is larger than Z resolution			
		if (ZCut>AtlasResZ) {
			Uncertainty = ZCut/AtlasResZ;
			Z=Z-Uncertainty/2;
			Randomizer = Uncertainty * random;
	...
	OpenAsHiddenResults(AtlasDir + "Segments_Reference.csv");
	...
	run("Set Measurements...", "mean redirect=None decimal=3");
	MeasurementCount=roiManager("count");	
	...
	//Create a annotated table -- update later to include region names
	title1 = "Annotated_Counts"; 
	title2 = "["+title1+"]"; 
	run("New... ", "name="+title2+" type=Table"); 
	print(title2,"\\Headings:X\tY\tZ\tZ_Dither\tMean_Int_Ch1\tMean_Int_Ch2\tMean_Int_Ch3\tMean_Int_Ch4\tHemisphere\tRegion_ID\tSegment"); 
	...
	for(i=0; i<ChCount; i++){ 
		...
		//Correction for cells located outside the range of the atlas (should only be a few - if many they will be visualized as densities at the edge of the volumes)
		ZPosition = ZPos[i];
		if (ZPosition > SegmentRefArray.length-1){
			ZPosition = SegmentRefArray.length-1;
		}
		
		Segment = SegmentRefArray[ZPosition];
		
		print(title2,XPos[i]+"\t"+YPos[i]+"\t"+ZPos[i]+"\t"+Z_Dither[i]+"\t"+MeanIntCh1[i]+"\t"+MeanIntCh2[i]+"\t"+MeanIntCh3[i]+"\t"+MeanIntCh4[i]+"\t"+HemisphereArray[i]+"\t"+MeanMeasurements[i]+"\t"+Segment);
	} 
	selectWindow(title1);
	...
	run("Text...", "save=["+ AnnotatedOut +"]");
	close(title1);


	// Create Basic Summary	
	print("  Creating atlas region summary table for channel "+CellChans[Ch_i]+" ...");
	// Then do region counts
	// Update to include segment counts	
	...
	title2 = "["+title1+"]"; 
	run("New... ", "name="+title2+" type=Table"); 
	print(title2,"\\Headings:Total_Cells_Right\tTotal_Cells_Left\tRegion_ID\tRegion_Acronym\tRegion_Name_1\tRegion_Name_2\tRegion_Name_3\tParent_ID\tParent_Acronym"); 
	...
	for(i=0; i<RegionNames.length; i++){ 
		...
		RegionAcronym = getResultString("acronym", i);
		RegionAcronym = replace(RegionAcronym, ",", " ");
		//print(f,0+"\t"+0+"\t"+RegionIDs[i]+"\t"+RegionAcronym+"\t"+RegionNamesArray[0]+"\t"+RegionNamesArray[1]+"\t"+RegionNamesArray[2]+"\t"+getResultString("parent_ID", i)+"\t"+getResultString("parent_acronym", i));
	
		print(title2,RegionCountsRight[i]+"\t"+RegionCountsLeft[i]+"\t"+RegionIDs[i]+"\t"+RegionAcronym+"\t"+RegionNamesArray[0]+"\t"+RegionNamesArray[1]+"\t"+RegionNamesArray[2]+"\t"+getResultString("parent_ID", i)+"\t"+getResultString("parent_acronym", i));
	} 
	selectWindow(title1);
	//Find way of renaming table to results so that it can be edited as results. For NOW just save and reopen
	//IJ.renameResults(title1,"Results");
	run("Text...", "save=["+ AnnotatedSummary +"]");
	close(title1);


// Create Segment Summary
	print("  Creating atlas segment summary table for channel "+CellChans[Ch_i]+" ...");

	//Create Empty Results table to add counts
	title1 = "Annotated_Summary"; 
	title2 = "["+title1+"]";  
	run("New... ", "name="+title2+" type=Table"); 
	print(title2,"\\Headings:C1_R\tC1_L\tC2_R\tC2_L\tC3_R\tC3_L\tC4_R\tC4_L\tC5_R\tC5_L\tC6_R\tC6_L\tC7_R\tC7_L\tC8_R\tC8_L\tT1_R\tT1_L\tT2_R\tT2_L\tT3_R\tT3_L\tT4_R\tT4_L\tT5_R\tT5_L\tT6_R\tT6_L\tT7_R\tT7_L\tT8_R\tT8_L\tT9_R\tT9_L\tT10_R\tT10_L\tT11_R\tT11_L\tT12_R\tT12_L\tT13_R\tT13_L\tL1_R\tL1_L\tL2_R\tL2_L\tL3_R\tL3_L\tL4_R\tL4_L\tL5_R\tL5_L\tL6_R\tL6_L\tS1_R\tS1_L\tS2_R\tS2_L\tS3_R\tS3_L\tS4_R\tS4_L\tCo1_R\tCo1_L\tCo2_R\tCo2_L\tCo3_R\tCo3_L\tRegion_R\tRegion_L\tRegion_Total\tRegion_ID\tRegion_Acronym\tRegion_Name_1\tRegion_Name_2\tRegion_Name_3\tParent_ID\tParent_Acronym"); 
	...
	for(i=0; i<RegionNames.length; i++){ 
		...
		print(title2,0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+0+"\t"+RegionIDs[i]+"\t"+RegionAcronym+"\t"+RegionNamesArray[0]+"\t"+RegionNamesArray[1]+"\t"+RegionNamesArray[2]+"\t"+getResultString("parent_ID", i)+"\t"+getResultString("parent_acronym", i));

	} 
	run("Text...", "save=["+ AnnotatedSegmentSummary +"]");
	close(title1);
	close("Results");
	
	// Update Table with counts
	open(AnnotatedSegmentSummary);
	Table.rename(Table.title, "Results");
	...
	for(j = 0; j< MeanMeasurements.length; ++j){
		...
		Segment = SegmentRefArray[ZPosition];
		Region =  MeanMeasurements[j];
		Hemisphere = HemisphereArray[j];
		SCTableCounter(Region, Segment, Hemisphere, "Results");	
	}
	...
	for (k=0; k<SegmentArray.length; k++){
		...
		L = parseInt(Table.get(SegmentArray[k]+"_L", i, "Results"));
		TotL = TotL+L;
		//Process Right Segments
		R = parseInt(Table.get(SegmentArray[k]+"_R", i, "Results"));
		TotR = TotR+R;
		//Set the totals
		}
		if (TotR > 0) {
			Table.set("Region_R", i, TotR, "Results");
		}
		if (TotL > 0) { 
			Table.set("Region_L", i, TotL, "Results");
		}
		if (TotL+TotR > 0 ) {
			Value = TotL+TotR;
	...
	//selectWindow(title1);
	run("Text...", "save=["+ AnnotatedSegmentSummary +"]");
	close("C"+CellChans[Ch_i]+"_Detected_Cells_Segment_Summary.csv");
}	
...
function MeasureIntensitiesLRHemisphereFullRes(Channel) {
	...
	// Then measure volume of regions in cropped annotated brain - check if it's already been measured
		...
		if (File.exists(input+"/5_Analysis_Output/Annotated_Volumes_XY_"+ProAnRes+"_Z_"+ZCut+"micron.csv")) {
			...
			f=title2; 
			run("New... ", "name="+title2+" type=Table"); 
			print(f,"\\Headings:Volume_Right\tVolume_Left\tRegion_ID"); 
		...
		f=title2; 
		run("New... ", "name="+title2+" type=Table"); 
		print(f,"\\Headings:Region_Volume_Right\tRegion_Volume_Left\tTotal_Intensity_Right\tTotal_Intensity_Left\tMean_Intensity_Right\tMean_Intensity_Left\tRegion_ID\tAcronym\tRegion_Name_1\tRegion_Name_2\tRegion_Name_3\tParent_ID\tParent_Acronym"); 
		...
		for(i=0; i<RegionNames.length; i++){ 
			...
			// what if region only has one name? will it cause error?
			...
			RegionAcronym = getResultString("acronym", i);
			RegionAcronym = replace(RegionAcronym, ",", " ");
			print(f,VolumesRight[i]+"\t"+VolumesLeft[i]+"\t"+ExpIntensityRight[i]+"\t"+ExpIntensityLeft[i]+"\t"+MeanIntensityRight+"\t"+MeanIntensityLeft+"\t"+RegionIDs[i]+"\t"+RegionAcronym+"\t"+RegionNamesArray[0]+"\t"+RegionNamesArray[1]+"\t"+RegionNamesArray[2]+"\t"+getResultString("parent_ID", i)+"\t"+getResultString("parent_acronym", i));
		} 
		close("Results");
		selectWindow(title1);
		//Find way of renaming table to results so that it can be edited as results. For NOW just save and reopen
...
function MeasureIntensitiesLRHemisphereFullResSpinalCord(Channel) {
	...
	if (File.exists(IntOutputDir+"C"+Channel+"_Mean_Region_and_Segment_Intensity.csv")) {
		...
		for(i=0; i<nResults; i++) {
			TransSegmentRefArray[i] = getResultString("Segment", i);
		}
		close("Results");
		...
		// Create Empty Table
		CreateEmptySpinalCordTable(IntOutputDir+"C"+Channel+"_Total_Region_and_Segment_Intensity.csv");
		//CreateEmptySpinalCordTable(IntOutputDir+"C"+Channel+"_Mean_Region_Intensity.csv");

		a = File.copy(IntOutputDir+"C"+Channel+"_Total_Region_and_Segment_Intensity.csv", IntOutputDir+"C"+Channel+"_Mean_Region_and_Segment_Intensity.csv");

		// Update Table with Volumes
		OpenAsHiddenResults(IntOutputDir+"C"+Channel+"_Total_Region_and_Segment_Intensity.csv");
		...
		for (j=0; j<NumIDs; j++) {
			...
			for (slice=1; slice<=IMslices; slice++) { 
				setSlice(slice); 
				getRawStatistics(n, mean, min, max, std, hist); 
				//SCVolumeMeasure is (Region, Segment,                      Hemisphere, Volume, Tablename)
				SCVolumeMeasure(RegionIDs[j], TransSegmentRefArray[slice-1], "Right", n*mean, "Results");
			}
			close("Result of RawIntensityData");
			...
			for (slice=1; slice<=IMslices; slice++) { 
				...
				getRawStatistics(n, mean, min, max, std, hist); 
				//SCVolumeMeasure is (Region, Segment,                      Hemisphere, Volume, Tablename)
				SCVolumeMeasure(RegionIDs[j], TransSegmentRefArray[slice-1], "Left", n*mean, "Results");	
		...
		close("*");
		selectWindow("Results");
		run("Text...", "save=["+ IntOutputDir+"C"+Channel+"_Total_Region_and_Segment_Intensity.csv]");
		close("C"+Channel+"_Total_Region_and_Segment_Intensity.csv");

		// Create Summary Rows for all regions
		SCCreateSummaryRows (IntOutputDir+"C"+Channel+"_Total_Region_and_Segment_Intensity.csv");
...
function CreateABADensityHeatmapJS(Chan) {
	...
	if (File.exists(ProjectionDensityOut + "/C"+Chan+"_Region_Density_Heatmap.tif")) {
		...
		run("Region Density Heatmap Creation SJ", "select=["+input+"] select_0=["+ProjectionDensityOut+"] select_1=["+AtlasDir+"] cell="+Chan+" atlassizex="+AtlasSizeX+" atlassizey="+AtlasSizeY+" atlassizez="+AtlasSizeZ);
...
function CreateABAIntensityHeatmapJS(Chan) {
	...
	if (File.exists(MeanIntensityOut + "/C"+Chan+"_Region_Intensity_Heatmap.tif")) {
		...
		run("Region Intensity Heatmap Creation SJ", "select=["+input+"] select_0=["+MeanIntensityOut+"] select_1=["+AtlasDir+"] cell="+Chan+" atlassizex="+AtlasSizeX+" atlassizey="+AtlasSizeY+" atlassizez="+AtlasSizeZ);
...
//
	...
	if (File.exists(input+"/5_Analysis_Output/Annotated_Volumes_XY_"+ProAnRes+"_Z_"+ZCut+"micron.csv")) {
		...
		f=title2; 
		run("New... ", "name="+title2+" type=Table"); 
		print(f,"\\Headings:Region_Volume_Right\tRegion_Volume_Left\tProjection_Volume_Right\tProjection_Volume_Left\tProjection_Density_Right\tProjection_Density_Left\tRelative_Density_Right\tRelative_Density_Left\tRegion_ID\tAcronym\tRegion_Name_1\tRegion_Name_2\tRegion_Name_3\tParent_ID\tParent_Acronym"); 
		...
		for(i=0; i<RegionNames.length; i++){ 
			...
			RegionAcronym = getResultString("acronym", i);
			RegionAcronym = replace(RegionAcronym, ",", " ");
			print(f,VolumesRight[i]+"\t"+VolumesLeft[i]+"\t"+ExpVolumesRight[i]+"\t"+ExpVolumesLeft[i]+"\t"+DensityRight+"\t"+DensityLeft+"\t"+RelativeDensityRight+"\t"+RelativeDensityLeft+"\t"+RegionIDs[i]+"\t"+RegionAcronym+"\t"+RegionNamesArray[0]+"\t"+RegionNamesArray[1]+"\t"+RegionNamesArray[2]+"\t"+getResultString("parent_ID", i)+"\t"+getResultString("parent_acronym", i));
		} 
		close("Results");
		selectWindow(title1);
		//Find way of renaming table to results so that it can be edited as results. For NOW just save and reopen
...
function CreateDensityTableLRHemisphereFullResSpinalCord(Channel, ProBGround) {
	// creates a volume and density table using a transformed annotation image
	// Transformed Annotations scaled up to match FR data
	// Isolate each region and perform density analysis
	// CreateDensityTable(input + "5_Analysis_Output/Transform_Parameters/OriginPoints/Origin_Output_Points.csv", ProCh, input + "5_Analysis_Output/Temp/Transformed_C"+ProCh+"_Binary_Out/result.mhd", input+"/5_Analysis_Output");
	...
	// Create image at Atlas Resolution in reg directory.
	print("   Creating binary dataset at "+AtlasResXY+" micron for transformation and visualization.");
	scaleX = FinalRes/AtlasResXY;
	run("Scale...", "x="+scaleX+" y="+scaleX+" interpolation=None process create");
	saveAs("Tiff",  RegDir+ "C"+Channel+"_25_Binary.tif");
	close();

	print("   Adjusting scale to make mesoscale mapping resolution:"+ProAnRes+" micron/pixel.");
	// Rescale if necessary 
	if (ProAnRes < FinalRes) {
		ProAnRes = FinalRes;
	}
	...
	saveAs("Tiff",  OutputDir + "/Projection_Density_Analysis_XY_"+ProAnRes+"_Z_"+ZCut+"micron/C"+Channel+"_projections_with_annotation_for_visualization.tif");
	close();
	...
	// Create Volume measurement table
	SCTableOut = OutputDir + "/Projection_Density_Analysis_XY_"+ProAnRes+"_Z_"+ZCut+"micron/C"+Channel+"_Region_and_Segment_Projection_Volume.csv";

	// Create empty tables for density and relative density
	CreateEmptySpinalCordTable(OutputDir + "/Projection_Density_Analysis_XY_"+ProAnRes+"_Z_"+ZCut+"micron/C"+Channel+"_Region_and_Segment_Projection_Density.csv");
	a = File.copy(OutputDir + "/Projection_Density_Analysis_XY_"+ProAnRes+"_Z_"+ZCut+"micron/C"+Channel+"_Region_and_Segment_Projection_Density.csv", OutputDir + "/Projection_Density_Analysis_XY_"+ProAnRes+"_Z_"+ZCut+"micron/C"+Channel+"_Region_and_Segment_Projection_Relative_Density.csv" );
	// 2) Open file to write into
	...
	for (j=0; j<NumIDs; j++) {
		...
		Region_Row = Region_Row + R_Total + "," + L_Total + "," + (R_Total+ L_Total) + "," + RegionIDs[j] + "," + RegionAcronyms[j]+","+RegionNames[j]+","+ParentIDs[j]+","+ParentAcronyms[j]+"\n";			
	...
	print("\\Update:   Density measurements complete.");
	}
}
